...
impl<G: GameClient> Infrastructure<G> {
            ...
            .update(&mut self.context.state, time_seconds)
        {
            match &inbound {
                &Update::Client(ClientUpdate::SessionCreated {
                    ...
                    let host = Context::<G>::compute_websocket_host(
                    ...
                    settings.set_cohort_id(Some(cohort_id), storages);
                    settings.set_server_number(server_number, storages);
                    //settings.set_arena_id(Some(arena_id), storages);
                    settings.set_player_id(Some(player_id), storages);
                    settings.set_token(Some(token), storages);
        ...
        match type_.as_str() {
            "keydown" | "keyup" => {
                ...
                if let Some(key) = Key::try_from_key_code(event.key_code()) {
                    ...
                    if !((e.ctrl || event.meta_key())
                        && matches!(e.key, Key::C | Key::F | Key::R | Key::V | Key::X))
                    {
    ...
    pub fn enable_rewarded_ads(&mut self) {
        self.context.client.rewarded_ads = true;
    }
    ...
    pub fn choose_server_id(&mut self, server_number: Option<ServerNumber>) {
        if server_number == self.context.common_settings.server_number {
            return;
        }
