...
#[derive(Copy, Clone, Default, Eq, PartialEq, Ord, PartialOrd, Hash, Encode, Decode)]
pub struct GenTicks<const FREQUENCY_HZ: TicksRepr>(pub TicksRepr);

pub trait TicksTrait {
    const ZERO: Self;
    const ONE: Self;
    const MAX: Self;
    const SECOND: Self;
    const FREQUENCY_HZ: TicksRepr;
    const PERIOD_SECS: f32 = 1.0 / (Self::FREQUENCY_HZ as f32);
}

impl<const FREQUENCY_HZ: TicksRepr> TicksTrait for GenTicks<FREQUENCY_HZ> {
    const ZERO: Self = Self(0);
    const ONE: Self = Self(1);
    const MAX: Self = Self(TicksRepr::MAX);
    const FREQUENCY_HZ: TicksRepr = FREQUENCY_HZ;
    const SECOND: Self = Self(FREQUENCY_HZ);
}
...
impl<const FREQUENCY_HZ: TicksRepr> GenTicks<FREQUENCY_HZ> {
    ...
    pub const FREQUENCY_HZ: Self = Self(FREQUENCY_HZ);
    pub const PERIOD_SECS: f32 = 1.0 / FREQUENCY_HZ as f32;

    /// Converts fractional seconds to a duration, which can be quite lossy.
    pub fn from_secs(secs: f32) -> Self {
        Self((secs * Self::SECOND.0 as f32) as TicksRepr)
    }

    /// Converts whole seconds to a duration.
    pub const fn from_whole_secs(secs: TicksRepr) -> Self {
        debug_assert!(
            secs.checked_mul(Self::SECOND.0).is_some(),
    ...
    pub const fn from_whole_millis(secs: u32) -> Self {
        let scaled = secs * Self::SECOND.0 as u32 / 1000;
