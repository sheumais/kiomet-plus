...
impl<G: GameArenaService> ChatRepo<G> {
            ...
            Self::minutes_to_instant(minutes, restrict_client.chat.context.restricted_until())
        {
            ...
            self.log_chat(
                ...
                &format!(
                    ...
                    req_player_id.0,
                    req_client.user_id,
                    restrict_client.alias,
                    restrict_player_id.0
                ),
                true,
                "executed",
            );
    ...
    fn send_chat(
        ...
        let team = service.get_team_members(req_player_id);
        ...
        let result = if let Some(req_client) = req_player.client_mut() {
            let recently_joined = req_client
                .metrics
                .visit_started
                .map(|vs| vs.elapsed() < Duration::from_secs(60 * 3))
                .unwrap_or(false);
            let options = ContextProcessingOptions {
                character_limit: NonZeroUsize::new(if !req_client.moderator && recently_joined {
                    75
                } else {
                    150
                }),
                safe_mode_until: self
                    .safe_mode_until
                    .filter(|_| !(req_client.moderator || whisper)),
        ...
        match result {
            Ok(text) => {
                ...
                let message = Arc::new(MessageDto {
                    ...
                    user_id: req_player.client().and_then(|c| c.user_id),
                    #[cfg(feature = "teams")]
                    ...
                    text,
                    whisper,
                    authentic: req_player
                        .client()
                        .and_then(|c| c.nick_name)
                        .map(|n| n.as_str() == alias.as_str())
                        .unwrap_or(false),
                });
                ...
                if whisper {
                    if let Some(team) = team {
                        #[cfg(feature = "teams")]
                        let members = team.members.iter();
                        #[cfg(not(feature = "teams"))]
                        let members = team.iter().copied();
