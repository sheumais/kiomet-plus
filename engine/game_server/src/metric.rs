...
use crate::net::ip::ip_to_region_id;
...
impl<G: GameArenaService> ClientMetricData<G> {
    pub(crate) fn new(auth: &Authenticate) -> Self {
        Self {
            ...
            renewed: auth.session_token.is_some(),
            created: Instant::now(),
            play_started: None,
            play_stopped: None,
            visit_started: None,
...
impl<G: GameArenaService> MetricRepo<G> {
    ...
    //const MINUTE_IN_MILLIS: u64 = 1 * 1000;
    ...
    pub fn start_visit(&mut self, client: &mut PlayerClientData<G>) {
        let renewed = client.metrics.renewed;
    ...
    fn update(infrastructure: &mut Infrastructure<G>) -> Option<(UnixTime, Bundle<Metrics>)> {
        ...
        let health = &mut infrastructure.health;
                ...
                if let Some(client) = player.client() {
                    ...
                    metrics_repo.mutate_with(
                        |m| {
                            ...
                            let retention_millis = now.saturating_sub(client.metrics.date_created);
                            let retention = (retention_millis as f64
                                * (1.0 / Self::DAY_IN_MILLIS as f64))
                                as f32;
        ...
        let mut general = |m: &mut Metrics| {
            m.world_size.push(world_size);
            m.entities.push(entities);
    ...
    pub fn get_metrics(
        ...
        for (_, context_service) in infrastructure.arenas.iter() {
            ...
            if filter.is_none() {
                metrics
                    .players_cached
                    .add_length(context_service.context.players.len());
                metrics
                    .sessions_cached
                    .add_length(context_service.context.players.real_players);
            }
        }
