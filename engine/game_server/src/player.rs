...
impl<G: GameArenaService> Default for PlayerRepo<G> {
    fn default() -> Self {
...
impl<G: GameArenaService> PlayerRepo<G> {
    ...
    pub(crate) fn update_is_alive_and_team_id(
        ...
        for pt in self.iter() {
            ...
            {
                use crate::team::PlayerTeamStatus;
                p.team.status = if let Some(team_id) = service.get_team_id(player_id) {
                    PlayerTeamStatus::teamed(team_id)
                } else {
                    PlayerTeamStatus::solo()
                }
            }
        }
    }
