...
impl<G: GameArenaService> TeamData<G> {
    ...
    pub fn is_member(&self, player_id: PlayerId) -> bool {
        debug_assert!(!self.members.is_empty(), "team shouldn't be empty");
    ...
    pub fn assign_captain(&mut self, player_id: PlayerId) -> bool {
        self.members.swap_to_front(player_id)
    }
}
...
pub struct PlayerTeamData {
    pub(crate) status: PlayerTeamStatus,
    #[cfg(feature = "teams")]
...
pub enum PlayerTeamStatus {
    ...
    Solo {
        /// Teams player is requesting to join. Is a VecDeque so old joins can be removed.
        #[cfg(feature = "teams")]
...
impl PlayerTeamStatus {
    ...
    pub(crate) fn teamed(team_id: TeamId) -> Self {
        Self::Teamed { team_id }
    }
}
...
#[cfg(feature = "teams")]
impl<G: GameArenaService> Default for TeamRepo<G> {
    fn default() -> Self {
        Self {
            teams: HashMap::new(),
...
mod test {
    ...
    fn team_repo() {
        ...
        let mut teams = TeamRepo::<MockGame>::default();
