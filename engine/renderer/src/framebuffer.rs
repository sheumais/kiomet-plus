...
impl Framebuffer {
    ...
    pub fn with_format(
        ...
        debug_assert!(matches!(
            format,
            TextureFormat::RgbaF16 | TextureFormat::RgbaF32
        ));
        Self::new2(renderer, background_color, linear_filter, format, false)
    }
...
impl<'a> FramebufferBinding<'a> {
    ...
    pub fn clear(&self) {
        ...
        let color = if self._framebuffer.srgb {
            crate::rgba_array(self._framebuffer.background_color)
        } else {
            color
        };
