...
pub trait MeshId: Copy + Clone + Hash + Eq + Ord {}
impl<T: Copy + Clone + Hash + Eq + Ord> MeshId for T {}
...
pub struct InstanceLayer<V, I, M, ID> {
    buffers: FxHashMap<ID, Buffers<V, I, M>>,
    sorted_ids: Vec<ID>,
}
        ...
        self.buffers
            ...
            .or_insert_with(
                ...
                || {
                    ...
                    // ever added, however for reasonable ammount of ids (< 1000) it probably doesn't matter.
...
impl<M: Vertex> DefaultRender for InstanceBuffer<M> {
...
impl<M: Vertex> InstanceBuffer<M> {
    ...
    /// Returns the number of instances in the [`InstanceBuffer`].
