...
impl<L: Layer> RenderChain<L> {
    ...
    /// Gets the [`Layer`] passed to [`new`][`Self::new`].
    pub fn layer(&self) -> &L {
        #[cfg(not(feature = "srgb"))]
        return &self.layer;
        #[cfg(feature = "srgb")]
...
pub struct Renderer {
    ...
    #[cfg(feature = "webgl2")]
    max_array_texture_layers: Cell<Option<u32>>,
    #[cfg(feature = "webgl2")]
    max_samples: Cell<Option<i32>>,
    max_texture_size: Cell<Option<u32>>,
...
impl Renderer {
    ...
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            anisotropy: None,
            #[cfg(feature = "webgl2")]
            max_array_texture_layers: Default::default(),
            #[cfg(feature = "webgl2")]
            max_samples: Default::default(),
            max_texture_size: Default::default(),
...
impl Renderer {
    #[cfg(feature = "webgl2")]
    pub(crate) fn max_array_texture_layers(&self) -> u32 {
        self.max_array_texture_layers
            .update(|prev| {
                Some(prev.unwrap_or_else(|| {
                    self.gl
                        .get_parameter(Gl::MAX_ARRAY_TEXTURE_LAYERS)
                        .unwrap()
                        .as_f64()
                        .unwrap() as u32
                }))
    ...
    pub fn enable_color_buffer_float(&mut self) {
        // We only need to enable them, not save them.
        #[cfg(not(feature = "webgl2"))]
        {
            // TODO could split up these 2 extensions into separate features.
            self.gl
    ...
    /// Reports whether renderer was created with antialiasing.
    ...
    pub fn set_depth_mask(&self, mask: bool) {
        self.gl.depth_mask(mask)
    }

    /// Sets the color mask and depth mask at the same time. TODO stencil mask?
    #[doc(hidden)]
    pub fn set_write_mask(&self, mask: bool) {
        self.set_color_mask(mask);
        self.set_depth_mask(mask);
    }
