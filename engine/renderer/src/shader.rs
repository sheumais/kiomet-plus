...
impl Uniform for &Texture {
    fn uniform(self, shader: &ShaderBinding, name: &'static str) {
        ...
        if shader.is_bound(index) {
            TextureBinding::drop_raw_parts(shader.renderer, index, shader.texture_type(index));
        }

        // Use a bitset instead of a Vec<TextureBinding>.
        mem::forget(self.bind(shader.renderer, index));
        shader.set_texture_type(index, self.typ())
    }
}
...
impl<'a> ShaderBinding<'a> {
    ...
    /// Returns if a specific texture index is bound.
    fn is_bound(&self, index: usize) -> bool {
        let bit = 1 << index;
        self.bound_textures.get() & bit != 0
    }

    /// Gets the [`TextureType`] of a specific index.
    fn texture_type(&self, index: usize) -> TextureType {
        debug_assert!(self.is_bound(index));
        let bit = 1 << index;
    ...
    /// Sets the [`TextureType`] of a specific index.
    fn set_texture_type(&self, index: usize, typ: TextureType) {
        let bit = 1u32 << index;
        self.bound_textures.update(|v| v | bit);
    ...
    fn drop(&mut self) {
        ...
        for index in 0..32 {
            ...
            if bitset & bit != 0 {
                ...
                TextureBinding::drop_raw_parts(self.renderer, index, self.texture_type(index));
            }
        }
    }
}
...
fn parse_attributes(vertex_source: &str) -> impl Iterator<Item = (&str, u32)> {
    ...
    if vertex_source.starts_with("#version 300 es") {
        ...
        // beneficial because those don't require calling bind_attrib_location.
        attribute = "in";
    }
