...
impl Camera2d {
    ...
    /// Convert a vector in view space to world space.
    pub fn to_world_vector(&self, view_vector: Vec2) -> Vec2 {
        self.camera_matrix.transform_vector2(view_vector)
    }
    ...
    /// Convert a vector in world space to view space.
    pub fn to_view_vector(&self, world_vector: Vec2) -> Vec2 {
    ...
    pub fn to_client_position(&self, world_position: Vec2) -> IVec2 {
        ...
        let zero_to_one = (self.to_view_position(world_position) + 1.0)
            * (0.5 / js_hooks::window().device_pixel_ratio() as f32);
        (zero_to_one * self.viewport.as_vec2()).as_ivec2()
    }
...
/// Useful for calculating meters per pixel and pixels per meter.
