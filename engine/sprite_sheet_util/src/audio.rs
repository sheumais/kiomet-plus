...
pub struct Sound {
    ...
    /// Treat as music from a volume perspective.
    pub music: bool,
    /// Trim start seconds.
    pub start: f32,
    /// Trim end seconds.
    pub end: f32,
    /// Play again and again instead of just once.
    pub looping: bool,
...
fn read_raw_audio(src: &str, sound: &Sound, channels: usize, sample_rate: usize) -> Vec<u8> {
    ...
    // Seek in input file.
    if sound.start > 0.0 {
        command.arg("-ss").arg(format!("{}", sound.start));
    }

    if sound.end.is_finite() {
        command.arg("-to").arg(format!("{}", sound.end));
    }
