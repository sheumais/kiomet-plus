...
pub fn pack_sprite_sheet(
                    ...
                    image.map(|image| {
                        ...
                        if params.width == 0 {
                            ...
                            let mut image = resize(
                                &image,
                                params.width,
                                ((params.width as f32 / aspect).round() as u32).max(1),
                                FilterType::Lanczos3,
                            );
                            if let Some(f) = output.pre_process {
                                image.pixels_mut().for_each(|pixel| {
    ...
    let total_area: u32 = sorted.values().map(|v| v.width * v.height).sum();
    ...
    let sizes: Box<dyn Iterator<Item = UVec2>> = if power_of_two {
        ...
        Box::new((min_pow2..=MAX_SIZE.ilog2()).flat_map(move |power| {
        ...
        Box::new((min_size..=(MAX_SIZE)).map(move |power| UVec2::splat(power * size_step)))
    };
